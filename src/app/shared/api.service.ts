...
export class ApiService {
  ...
  requestNetto1(): Observable<string> {
    ...
    hdr.append('Access-Control-Allow-Origin', 'http://localhost/3000');
  ...
  pushEventData(value: string): Observable<ApiRetValue> {
    if (this.AspNet) {
      return this.http
        .post<ApiRetValue>('/api/Data/PushEventData', value, JsonOptions)
        .pipe(catchError(this.handleError));
    }
    return this.http
      .post<ApiRetValue>('/api/event-data', value, JsonOptions)
      .pipe(catchError(this.handleError));
  }
